...
exports.create = function(tableName, options) {
  ...
  var checkRotate = createCheckRotate(tableName, basePath, maxDataFileSize);
...
function createCheckRotate(tableName, basePath, maxDataFileSize) {
  return function(writeStream, callback) {
    ...
    } else if (maxDataFileSize) {
      return fs.stat(filename, function(err, stats) {
        if (err) {
          console.error('could not stat %s, rotating file', filename);
          return callback(true);
        } else {
          return callback(stats.size >= maxDataFileSize);
        }
      });
    } else {
