...
exports.create = function(tableName, options) {
  ...
  var maxDataFileSize = options.maxDataFileSize;
  ...
  var pipeline = eventstream.pipeline(
    ...
    rotatingstream.create(streamFactory, checkRotate, rotateInterval, maxDataFileSize));
...
exports.shutdown = function(batchelors, callback) {
  var pipeline = eventstream['pipeline'].apply(this, batchelors);
  pipeline.on('end', callback);
  pipeline.on('error', callback);
  pipeline.end();
  return pipeline;
};
...
function createCheckRotate(tableName, basePath) {
  return function(writeStream, callback) {
    var filename = datafileName(tableName, basePath);
    var shouldRotate = filename !== curFilenames[tableName];
    return callback(shouldRotate);
  };
};

function datafileName(tableName, basePath) {
  return basePath + '/' + tableName + '.' + new Date().getTime() + '.' + process.pid + '.data';
};
